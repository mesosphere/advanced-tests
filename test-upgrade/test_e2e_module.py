...
def ee_artifact_url() -> str:
    """
    FIX: Return the path to a DC/OS build artifact to test against.
    """
    generate_config_url = str(os.environ['EE_DCOS_E2E_GENCONF_URL'])
    return generate_config_url


@pytest.fixture(scope='session')
def ee_upgrade_artifact_url() -> str:
    """
    FIX: Return the path to a DC/OS build artifact to test against.
    """
    generate_config_upgrade_url = str(os.environ['EE_DCOS_E2E_GENCONF_UPGRADE_URL'])
    return generate_config_upgrade_url
